...
export default function AdminSidebar({
  ...
  return (
    <>
      ...
      {/* Sidebar Drawer */}
      <aside
        className={`fixed top-0 bottom-0 left-0 z-50 flex flex-col bg-slate-900/98 backdrop-blur-2xl border-r border-slate-800/90 text-slate-200 shadow-2xl transition-all duration-300 ${
      ...
      >
        ...
        <div className="flex items-center justify-between h-16 px-4 border-b border-slate-800/80 bg-slate-950/60">
          <Link href="/admin" className="flex items-center gap-3 overflow-hidden group">
            <div className="flex items-center justify-center w-9 h-9 rounded-xl bg-gradient-to-tr from-orange-500 via-amber-500 to-orange-600 text-white font-black text-base shadow-lg shadow-orange-500/30 shrink-0 group-hover:scale-105 transition-transform">
              MJ
            </div>
            {!collapsed && (
              <div className="flex flex-col truncate">
                <span className="font-bold text-slate-100 text-sm tracking-tight flex items-center gap-1">
                  MaaJanki <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                </span>
                <span className="text-[11px] text-orange-400 font-semibold tracking-wide">Web Tech Admin</span>
              </div>
            )}
          </Link>
          ...
          <button
            ...
            className="hidden lg:flex items-center justify-center w-8 h-8 rounded-lg text-slate-400 hover:text-slate-100 hover:bg-slate-800/80 transition-colors"
        ...
        <div className="flex-1 overflow-y-auto px-3 py-4 space-y-6 sidebar-scroll">
          {navSections.map((section, sectionIdx) => (
            <div key={sectionIdx} className="space-y-1">
              {!collapsed ? (
                <div className="px-3 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-1.5 opacity-90">
                ...
                return (
                  <div key={item.id} className="space-y-1 relative group">
                    ...
                    {hasSubmenu && !collapsed && isSubOpen && (
                      <div className="pl-6 pr-1 py-1 space-y-1 border-l-2 border-slate-800/90 ml-5 my-1">
        ...
        <div className="p-3 border-t border-slate-800/80 bg-slate-950/50">
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold text-rose-400 hover:text-rose-300 hover:bg-rose-500/15 transition-colors border border-rose-500/20"
