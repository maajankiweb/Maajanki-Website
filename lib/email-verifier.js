...
export async function verifyEmailAddress(email) {
  ...
  // 1b. Dummy Username & Domain Filter
  const DUMMY_USERNAMES = new Set([
    'test', 'testing', 'tester', 'admin', 'administrator', 'root', 'user', 'dummy', 'fake',
    'asdf', 'asdfgh', 'qwerty', 'xyz', 'abc', 'abcd', 'none', 'noemail', 'null', 'demo',
    'sample', 'example', '123456', '12345', '111111', '000000', 'temp', 'spam'
  ]);
  const DUMMY_DOMAINS = new Set([
    'test.com', 'example.com', 'sample.com', 'domain.com', 'fake.com', 'email.com',
    'xyz.com', 'abc.com', 'asdf.com', 'none.com', 'test.in', 'example.in', 'demo.com'
  ]);

  if (DUMMY_USERNAMES.has(username) || DUMMY_DOMAINS.has(domain) || /^(\w)\1{3,}$/.test(username)) {
    return {
      valid: false,
      reason: 'Dummy ya fake email address swikaar nahi hai. Kripya apna genuine email id darj karein.'
    };
  }
